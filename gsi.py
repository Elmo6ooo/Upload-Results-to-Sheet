...
with open("/usr/local/google/home/chienliu/Downloads/android-cts/results/2022.09.02_02.26.07/test_result_failures_suite.html") as fp:
    soup = BeautifulSoup(fp, 'html')

scopes = ["https://spreadsheets.google.com/feeds"]
credentials = ServiceAccountCredentials.from_json_keyfile_name("credentials.json", scopes)
client = gspread.authorize(credentials)
sheet = client.open_by_key("1eSp4g_2Z86MnZLUVljhMoDt1OgRIPeEV0KYMVq22zag").worksheet("TM-Seahawk-CTS-on-GSI")	
...
for i in range(0, len(result), 7):
	...
	if result[i] in dic:
		...
		if result[i+2] != '0' and result[i+6] == 'true':
			...
			cells.append(Cell(dic[result[i]], 8, 'FAIL'))
		elif result[i+6] == 'false':
			cells.append(Cell(dic[result[i]], 8, 'INCOMPLETE'))
		elif result[i+3] == result[i+4] and result[i+3] == result[i+5]:
			cells.append(Cell(dic[result[i]], 8, 'REMOVED'))
		elif result[i+4] == result[i+5] and result[i+4] != '0':
			cells.append(Cell(dic[result[i]], 8, 'Ignored'))
		elif result[i+3] != '0' and (result[i+3] == result[i+5] or int(result[i+3])+int(result[i+4]) == int(result[i+5])):
			cells.append(Cell(dic[result[i]], 8, 'Assumption'))
		else:
			cells.append(Cell(dic[result[i]], 8, 'PASS'))		
	else:
